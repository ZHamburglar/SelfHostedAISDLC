 ...
 * Issue Coder Agent
 *
 * Triggered when a bug, feature, or hotfix label is added to a GitHub issue.
 * - Reads the issue title, body, labels, repo structure, README, and AGENTS.md guidelines
 * - Runs a multi-stage LM Studio pipeline (planning → drafting → validation) to produce a file change plan
 * - Creates a new branch (agent/issue-<number>-<run-id>), applies the planned file changes, commits, and pushes
 * - Opens a pull request with a full breakdown of the work completed
 * - Comments on the original issue linking back to the new PR
 * - Posts instructions on the PR for requesting further changes via the PR Revision Agent (/fix)
 */
...
const LM_STUDIO_TIMEOUT_MS = parsePositiveIntEnv("LM_STUDIO_TIMEOUT_MS", 900000);
const LM_DEFAULT_STAGE_MAX_TOKENS = 1200;
const LM_STAGE_MAX_TOKENS = {
  planning: parsePositiveIntEnv("LM_STUDIO_PLANNING_MAX_TOKENS", 700),
  drafting: parsePositiveIntEnv("LM_STUDIO_DRAFTING_MAX_TOKENS", 900),
  validation: parsePositiveIntEnv("LM_STUDIO_VALIDATION_MAX_TOKENS", 1800),
};
...
- Keep "changes" scoped and minimal.`;
  ...
  // 7. Build the PR body from the template + agent work breakdown
  const { file: templateFile } = detectPRTemplate();
  const templateContent = readTemplate(templateFile);

  const workBreakdown = plan.changes.length === 1
    ? `**Change made:**\n- \`${plan.changes[0].action}\` \`${plan.changes[0].path}\` — ${plan.changes[0].reason}`
    : `**Changes made:**\n${plan.changes.map((c) => `- [x] \`${c.action}\` \`${c.path}\` — ${c.reason}`).join("\n")}`;

  const prBody = `${templateContent}

---
> 🤖 *This PR was automatically generated by the Issue Coder Agent for issue #${ISSUE_NUMBER}.*
